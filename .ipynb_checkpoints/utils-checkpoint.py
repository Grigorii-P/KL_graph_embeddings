...
def create_equal_portinons(all_labels_path, ready_graphs_path, dst_file):
    with open(all_labels_path, 'r') as file:
        all_labels = json.loads(file.read())
        
    with open(ready_graphs_path) as f:
        content = f.readlines()
    ready_graphs = [x.split('/')[1].split('.')[0] for x in content]
